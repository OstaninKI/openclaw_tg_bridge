...
const DM_TARGET_PREFIXES = [CHANNEL_ID, "tguser", "tgdm"] as const;
...
function normalizeChannelAccount(
  ...
  return {
    ...
    typingMaxDurationMs:
      typeof raw.typingMaxDurationMs === "number" && raw.typingMaxDurationMs > 0
        ? raw.typingMaxDurationMs
...
function withProfileParameter(parameters: Record<string, unknown>): Record<string, unknown> {
  return {
    ...parameters,
    properties: {
    ...
    required: Array.from(new Set(["profile", ...((parameters.required as string[] | undefined) ?? [])])),
  };
}

function resolveConfiguredProfile(config: PluginConfig, profileId: unknown): ProfileConfig | null {
...
function createProfileTool(api: PluginApi, profile: ProfileConfig, toolName: string): RegisteredTool | null {
...
function registerStableProfileTools(api: PluginApi, config: PluginConfig): void {
  if (config.profiles.length !== 1) return;
  const templateTools: Array<{ tool: RegisteredTool; opts?: { optional?: boolean } }> = [];
  const templateApi = {
    ...api,
...
function shouldRegisterLegacyProfileAliases(profile: ProfileConfig): boolean {
...
function normalizeDmMessagingTarget(target: string): string {
  return stripDmTargetPrefix(target);
}

function looksLikeDmTargetId(raw: string, normalized?: string): boolean {
...
async function startInboundDmTypingLoop(params: {
  ...
  const stopInternal = async (options: { clearMaxDurationTimer?: boolean } = {}) => {
    ...
    if (clearMaxDurationTimer && maxDurationTimer !== null) {
  ...
  const maxDurationMs =
    typeof params.account.typingMaxDurationMs === "number" && params.account.typingMaxDurationMs > 0
      ? params.account.typingMaxDurationMs
      : DEFAULT_TYPING_MAX_DURATION_MS;
  maxDurationTimer = setTimeout(() => {
    void stopInternal({ clearMaxDurationTimer: false });
  }, maxDurationMs);
    ...
    config: {
      ...
      inspectAccount: (cfg: Record<string, unknown>, accountId?: string) => {
        ...
        return {
          ...
          typingMaxDurationMs: account.typingMaxDurationMs,
    ...
    gateway: {
      startAccount: async (ctx: GatewayStartContext) => {
        ...
        try {
          ...
          if (runningAccounts.get(ctx.account.accountId) === handle) {
            runningAccounts.delete(ctx.account.accountId);
          }
        }
